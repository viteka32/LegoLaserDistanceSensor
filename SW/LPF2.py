...
#----- comm stuff
          ...
          if self.connected:
               ...
               while chr >=0:
                    ...
                    if chr == 0:   # port has nto been setup yet
                         ...
                         self.watchdogTimer.deinit()
                         self.watchdogTimer = machine.Timer(-1, mode=machine.Timer.ONE_SHOT, period=1500, callback=self.wdgCallback )  # watchdog for 1500 ms
                         pass   
                    elif chr == CMD_Select:    # reset the mode
                         mode = int.from_bytes(self.uart.read(1), "big")
                         cksm = int.from_bytes(self.uart.read(1), "big")
                         print ("CMD_Select")
                         print (mode)
                         ...
                         zero = int.from_bytes(self.uart.read(1), "big")
                         b9 = int.from_bytes(self.uart.read(1), "big")
                         print ("0x46")
                         print (zero)
                         print (b9)
                         ck = 0xff ^ zero ^ b9
                         if ((zero == 0) & (b9 == 0xb9)):   # intro bytes for the string
                              char = int.from_bytes(self.uart.read(1), "big")    # size and mode
                              ...
                              for i in range(size):
                                   self.textBuffer[i] = int.from_bytes(self.uart.read(1), "big")
                                   ck = ck ^ self.textBuffer[i]
                              print (self.textBuffer)
                              print("".join("\\x%02x" % i for i in self.textBuffer))
                              cksm = int.from_bytes(self.uart.read(1), "big")
                              print(cksm)
                              print(ck)
                              if cksm == ck:
                         ...
                         thing = int.from_bytes(self.uart.read(1), "big")
                         cksm = int.from_bytes(self.uart.read(1), "big")
                         print ("0x4C")
                         print (thing)
     ...
     def wdgCallback(self, timerInfo):
          self.connected = False
...
# -----  Init and close
     ...
     def init(self):
          ...
          self.rx = machine.Pin(self.rxPin, machine.Pin.IN, machine.Pin.PULL_UP)
          self.tx.value(0)
          utime.sleep_ms(500)
          self.tx.value(1)
          self.uart.init(baudrate=2400, bits=8, parity=None, stop=1, tx=machine.Pin(self.txPin), rx=machine.Pin(self.rxPin))
          self.writeIt(b'\x00')
...
# -----   Start everything up
     ...
     def initialize(self):
          self.connected = False
          self.sendTimer = machine.Timer(self.txTimer )  # default is 200 ms
          self.watchdogTimer = machine.Timer(-1 )  # watchdog for 1500 ms
          self.init()
          ...
          if self.connected:
               ...
               #change baudrate
               self.uart.init(baudrate=115200, bits=8, parity=None, stop=1, tx=machine.Pin(self.txPin), rx=machine.Pin(self.rxPin))
               ...
               self.sendTimer.deinit()
               self.watchdogTimer.deinit()
               self.sendTimer = machine.Timer(self.txTimer, freq = self.freq, callback=self.hubCallback )  # default is 200 ms
               self.watchdogTimer = machine.Timer(-1, mode=machine.Timer.ONE_SHOT, period=1500, callback=self.wdgCallback )  # watchdog for 1500 ms
          return